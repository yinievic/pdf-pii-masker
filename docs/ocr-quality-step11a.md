# Version 11A OCR 품질 검증

Version 11A는 Version 12로 넘어가기 전 OCR 탐지 품질을 최소한으로 개선·검증하는 단계다. 운영 OCR API 응답 구조는 유지하고, 품질 비교는 별도 스크립트로 수행한다.
...
Version 11A에서는 운영 OCR API 응답 구조를 유지하면서 탐지 후처리를 먼저 보강한다.
